...
- 轻点节点只会选中；指针移动达到 4px 后才开始拖动。拖动默认吸附到 16px 网格和相邻节点对齐参考线，按住 `Alt` 可临时绕过吸附。
...
- 点击左侧“电商”进入电商宣传图工作台：上传商品图（必填），可选上传模特图和场景图；可以使用 Agnes 自动写提示词或直接手写提示词，结果可添加到画布或下载。
- 点击左侧“产品视频”进入产品视频工作台：上传产品图（必填），设置镜头提示词和视频参数后生成，结果可添加到画布或下载。
