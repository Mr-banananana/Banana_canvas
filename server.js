...
async function handleAgnesImage(req, res) {
  ...
  const commercePrompt = payload.workflow === "commerce"
    ? [
      prompt,
      payload.sceneOnly === true
        ? "scene-only：只生成有色彩、有层次、有商业摄影光影的背景与环境，不要生成商品、产品包装、人物手中的产品或任何重复主体。允许品牌色、渐变、真实道具、材质对比和生活方式场景，不要默认纯白背景。为后续叠加真实产品和中文文案预留自然构图空间。"
        : "商品身份锁定：第一张参考图是唯一权威商品，只允许改变背景、光线和构图，不得重设计、重新建模、拉伸、液化或改变商品比例。",
      payload.sceneOnly === true
        ? "不要生成任何可读文字、中文、英文、数字、字母、标签、标题、logo、水印或界面元素。"
        : "完整保留商品的外形轮廓、数量、结构、材质、颜色、接口、按键、包装、标识位置和可见细节；不得添加、删除、合并或替换部件。",
      payload.sceneOnly !== true ? "模特图和场景图仅用于人物展示方式、环境和光线，不能替换或改造商品；商品要清晰、完整、占主要视觉面积，避免手部遮挡和复杂动作。" : ""
    ].filter(Boolean).join("\n")
    : prompt;
...
async function handleAgnesPrompt(req, res) {
  ...
  const content = [
    {
      type: "text",
      text: [
        "请先识别商品类别、主体结构、材质、颜色、可见功能和视觉特色，再为它生成一张单屏电商海报的视觉提示词和中文排版文案；这是一次单张海报创作。",
        "参考图顺序与角色如下：",
        referenceSummary,
        ...
        "visualPrompt 只规划一张完整电商海报：一个统一场景、一个构图、明显的色彩和光影设计、适度留白和可放置中文文案的位置；视觉场景可以使用品牌色、渐变、材质道具和生活方式环境，不要默认纯白背景。",
        ...
        "只返回一段严格 JSON，不要 Markdown，不要解释。格式必须是：{\"visualPrompt\":\"一段可直接用于生图的场景视觉提示词\",\"copy\":{\"title\":\"不超过 18 个汉字的标题\",\"benefits\":[\"不超过 16 个汉字的卖点\",\"不超过 16 个汉字的卖点\",\"不超过 16 个汉字的卖点\"],\"specs\":[]}}。visualPrompt 不得包含需要生图模型绘制的中文文字；copy 只供网页文字层使用。"
      ].join("\n")
    }
  ];
  ...
  const body = {
    ...
    messages: [
      {
        role: "system",
        content: "你是电商海报视觉策划和中文排版文案专家。你必须基于图片证据提炼可验证的产品卖点，输出严格 JSON，其中 visualPrompt 只描述有色彩、有场景、有光影的单屏商业画面，copy 用于网页文字层；禁止编造不可见的产品参数或营销承诺；visualPrompt 必须要求场景不含产品重绘和任何可读文字。"
      },
      { role: "user", content }
    ],
    temperature: 0.78,
